...
export async function parsePdf(data){const {PDFParse}=await import('pdf-parse');const parser=new PDFParse({data:new Uint8Array(data),isEvalSupported:false});try{const result=await parser.getText({first:20});if(result.total>20)throw Error('PDF exceeds 20 pages. Split it before extraction.');const pages=result.pages.map(p=>({page:p.num,text:p.text.slice(0,30000)}));check(pages.length,'No pages could be read.');return {pages,claims:extractClaims(pages),method:'PDF text',needsOcr:pages.every(p=>p.text.trim().length<30)}}finally{await parser.destroy()}}
